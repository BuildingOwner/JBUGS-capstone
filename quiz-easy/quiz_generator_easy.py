...
from png2keyword_tesseract import png2keyword_tesserat
import json
...
def extrect_keyword(path):
    img_path = pdf2png(path)
    question = png2keyword_tesserat(img_path) # 모듈에 따라 스위칭
    question += """
...
def generator(keyword):
    ...
    question = f"""{keyword} 이 단어에 관한 객관식 문제와 그 문제의 선지와 답 쌍을 5개 생성해줘. 
    ...
    try:
        ...
        return generator(keyword)  # 재귀 호출로 다시 생성

    return result


if __name__ == "__main__":
    print(generator(extrect_keyword('학습자료/3-DL-개요.pdf')))
